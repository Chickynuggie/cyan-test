        ...
        return (<div className='account-list'>
            {this.props.pagedProfiles.map(profile => <div
                ...
                onClick={() => this.props.selectUser(profile.id.toString())}>
                    ...
                    <div className='account-list__card__info'><div>Country:</div><div className="text-overflow-ellipsis">{profile.country || 'not provided'}</div></div>
                    <div className='account-list__card__info'><div>City: </div><div className="text-overflow-ellipsis">{profile.city || 'not provided'}</div></div>
                    <div className='account-list__card__info'><div>Address: </div><div className="text-overflow-ellipsis">{profile.address || 'not provided'}</div></div>
            </div>)}
        </div>)
    };
}
